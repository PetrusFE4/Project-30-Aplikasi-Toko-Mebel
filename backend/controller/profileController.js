...
const updateProfile = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      updatedFields.image = `/uploads/${req.file.filename}`; // Gunakan path relatif dengan nama file yang diunggah
    }

    // Lakukan pembaruan hanya jika ada setidaknya satu nilai yang diubah
    const [results] = await db.execute('UPDATE tbl_users SET username = ?, email = ?, password = ?, no_hp = ?, image = ? WHERE id_user = ?', [updatedFields.username, updatedFields.email, updatedFields.password, updatedFields.no_hp, updatedFields.image, id_user]);
